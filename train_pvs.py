...
def train(train_data, val_data, model, device, criterion, lr = 0.01, EPOCHS=10, model_name='Model'):
    ...
    for epoch in range(EPOCHS):
        ...
        with torch.no_grad():
            ...
            print("val----epoch{}----avgloss{}---kl{}----cc{}----nss{}----auj{}----aub{}".format(epoch, (
...
if __name__ == '__main__':
    ...
    model = models0629.SST_Sal(hidden_dim=9)
    ...
    train_video360_dataset = RGB(config.frames_dir,  config.gt_dir, config.fixation_dir,config.train_set, config.sequence_length, resolution=config.resolution,transform=False)
    val_video360_dataset = RGB(config.frames_dir,  config.gt_dir, config.fixation_dir,config.test_set, config.sequence_length,  resolution=config.resolution)
