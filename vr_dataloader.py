...
class RGB(Dataset):
    def __init__(self, path_to_frames, path_to_saliency_maps, path_to_fixation_maps, video_names, frames_per_data=20,
                 split_percentage=0.2, split='train', resolution=[240, 320], skip=0, load_names=False, transform=False):
        self.sequences = []
            ...
            for end in range(skip + frames_per_data, len(video_frames_names) - 5, frames_per_data):
                ...
                if not self.path_frames is None:
                    ...
                    for frame in video_frames_names[sts:end]:
                        ...
                        if os.path.exists(os.path.join(self.path_frames, name, frame)):
                            bbbbbb= os.path.join(self.path_sal_maps, name, frame.split("_")[1])
                            if not os.path.exists(os.path.join(self.path_sal_maps, name, frame.split('_')[1])) :
    ...
    def __getitem__(self, idx):
        ...
        for frame_name in self.sequences[idx]:
            ...
            if not self.path_to_fixation_maps == None:
                ...
                assert os.path.exists(
                    fixation_map_path), 'Saliency map has not been found in path: ' + fixation_map_path

                fixation_map = cv2.imread(fixation_map_path, cv2.IMREAD_GRAYSCALE)

                if fixation_map.shape[1] != self.resolution[1] or fixation_map.shape[0] != self.resolution[0]:
                    fixation_map = cv2.resize(fixation_map, (self.resolution[1], self.resolution[0]),
                                              interpolation=cv2.INTER_AREA)
        ...
        if self.load_names:
            ...
            if self.path_sal_maps is None or self.path_to_fixation_maps is None:
                sample = [torch.cat(frame_img, 0), frame_names]
            else:
                sample = [torch.cat(frame_img, 0), torch.cat(label, 0), torch.cat(fixation, 0), frame_names]
